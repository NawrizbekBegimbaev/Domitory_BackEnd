...
class _DashboardScreenState extends State<DashboardScreen> {
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            Text(DateTime.now().toString().split(' ')[0], style: const TextStyle(color: AppColors.textMuted, fontSize: 12)),
            ...
            _wideStatCard('Задолженность', '${_formatMoney(_summary?['total_debt'] ?? 0)} ₽', AppColors.accent),
            const SizedBox(height: 12),
            _wideStatCard('Собрано за месяц', '${_formatMoney(_summary?['collected_this_month'] ?? 0)} ₽', AppColors.success),
  ...
  Widget _debtorRow(dynamic d) {
    return Container(
      ...
      child: Row(children: [
        CircleAvatar(radius: 18, backgroundColor: AppColors.accent.withAlpha(30),
          child: Text((d['full_name'] ?? '?')[0], style: const TextStyle(color: AppColors.accent, fontWeight: FontWeight.bold, fontSize: 14))),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(d['full_name'] ?? '', style: const TextStyle(fontWeight: FontWeight.w500, fontSize: 14)),
          Text('${d['faculty'] ?? ''}', style: const TextStyle(color: AppColors.textMuted, fontSize: 11)),
        ])),
        Text('${_formatMoney(d['debt'])} ₽', style: const TextStyle(color: AppColors.danger, fontWeight: FontWeight.bold, fontSize: 14)),
      ]),
    );
  }
}
